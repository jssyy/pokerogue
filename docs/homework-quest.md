...
管理页（`http://localhost:8001`）每一行有四个动作：

| 动作 | 会发生什么 |
| --- | --- |
| 改昵称 | 只改显示名。**登录名不动**——那是孩子要打的字，也是所有数据挂靠的键 |
| 改密码 | 改完这个孩子已登录的设备全部掉线，因为需要重置的密码通常是别人知道了 |
| 停用 / 恢复 | 暂时不能登录，**进度一点不动**，随时恢复 |
| 删除 | **存档、作业、体力流水一起没掉，不可恢复** |

删除要把登录名原样打一遍才执行。别处都偏向保住进度，唯一真正销毁进度的动作就不该只花一次误点的代价。

全部推倒重来（连家长账号一起）：停掉服务，删掉 `server/data/family.sqlite*` 三个文件，重启——下一个注册的账号又会成为家长。备份在 `server/data/backups/`，删错了可以从那里拷回来。

### 端口

| 端口 | 谁 | 定在哪 |
| --- | --- | --- |
| **8000** | 游戏（Vite） | `.env.family` 的 `VITE_PORT` |
| **8001** | 账号服务 + 管理页 | 服务端的 `PORT` 环境变量，默认 8001 |

游戏那边的端口被占用时，**Vite 会自己往后找**（8000 占了就用 8002），启动日志里 `Local: http://localhost:xxxx` 那一行才是实际地址——看那一行，不要想当然。

账号服务不会自己换端口，被占就直接报错。已经有一个在跑时用重启：

```bash
pnpm restart:server
```

它只在端口上那个东西用 `/health` 回答 `ok`（也就是确认是本服务）时才停掉它；被别的程序占着会如实报出来、不动手。
