...
describe("UI - Homework planner", () => {
  ...
  function chooseLabel(label: string): void {
    ...
    // Some labels interpolate a number the test has no reason to know; matching around the
    // placeholders keeps the lookup on the wording, which is the part that identifies the option.
    const escaped = label.split(/\{\{[^}]*\}\}/).map(part => part.replace(/[.*+?^${}()|[\]\\]/g, "\\$&"));
    const pattern = new RegExp(`^${escaped.join(".*")}$`);
    const index = menu.config.options.findIndex(o => pattern.test(o.label));
    expect(index, `no option matching "${label}"`).toBeGreaterThanOrEqual(0);
    chooseOption(index);
  }
  ...
  function openMainMenu(handler: HomeworkUiHandler, taskRows: number): void {
    ...
    expect(game.scene.ui.mode).toBe(UiMode.OPTION_SELECT);
  }
  ...
  function enterParentMode(handler: HomeworkUiHandler, taskRows: number): void {
    ...
    expect(game.scene.ui.mode).toBe(UiMode.HOMEWORK_PIN);
    ...
    expect(game.scene.ui.mode).toBe(UiMode.HOMEWORK);
  }
  ...
  function expectPlannerAlive(handler: HomeworkUiHandler): void {
    ...
    expect(game.scene.ui.mode).not.toBe(UiMode.HOMEWORK);
  ...
  async function waitForMode(mode: UiMode): Promise<void> {
    ...
    while (game.scene.ui.mode !== mode) {
      if (Date.now() > deadline) {
        throw new Error(`Never reached ${UiMode[mode]}; the mode is $UiMode[game.scene.ui.mode].`);
      }
  ...
  describe("as the home screen", () => {
    ...
    async function waitForHomeScreen(): Promise<HomeworkUiHandler> {
      const deadline = Date.now() + 10000;
      while (game.scene.ui.mode !== UiMode.HOMEWORK) {
        if (Date.now() > deadline) {
          throw new Error(`The planner never opened; the mode is ${UiMode[game.scene.ui.mode]}.`);
        }
        await new Promise(resolve => setTimeout(resolve, 50));
      }
    ...
    it("is where the game lands after the title phase, with a row to set out from", async () => {
      ...
      expect(handler.processInput(Button.ACTION)).toBe(true);
      expect(game.scene.ui.mode).toBe(UiMode.HOMEWORK_OPTION_SELECT);
    });
  ...
  describe("every menu action closes its loop", () => {
    ...
    describe("the set-out menu", () => {
      ...
      it("opens the settings", async () => {
        ...
        expect(game.scene.ui.mode).toBe(UiMode.SETTINGS_GENERAL);
        expect(plannerIsOnTop(handler)).toBe(false);
      });
    ...
    describe("parent tools", () => {
      it("toggles the stamina gate", async () => {
        ...
        openMainMenu(handler, 0);
        chooseLabel(i18next.t("homework:action.parentSettings"));
        chooseLabel(i18next.t("homework:parent.toggleGate"));
      ...
      it("grants a free pass for today only", async () => {
        ...
        openMainMenu(handler, 0);
        chooseLabel(i18next.t("homework:action.parentSettings"));
        chooseLabel(i18next.t("homework:parent.freePass"));
      ...
      it("grants stamina", async () => {
        ...
        openMainMenu(handler, 0);
        chooseLabel(i18next.t("homework:action.parentSettings"));
        chooseLabel(i18next.t("homework:parent.grantStamina"));
      ...
      it("changes the PIN, and the old one stops working", async () => {
        ...
        openMainMenu(handler, 0);
        chooseLabel(i18next.t("homework:action.parentSettings"));
        chooseLabel(i18next.t("homework:parent.changePin"));
        expect(game.scene.ui.mode).toBe(UiMode.HOMEWORK_PIN);
      ...
      it("adds a task with a title typed by hand", async () => {
        ...
        expect(game.scene.ui.mode).toBe(UiMode.HOMEWORK_TASK_FORM);
      ...
      it("deletes a task through the confirmation", async () => {
        ...
        expect(game.scene.ui.mode).toBe(UiMode.CONFIRM);
        chooseOption(0); // yes
